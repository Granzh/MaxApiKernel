...
Rust‑клиент для Max API: авторизация, сессии, сообщения, события и управление соединением.
Подходит как ядро/библиотека для встраивания в бэкенд или другие приложения.

## Дисклеймер (Disclaimer)

Это неофициальная библиотека на Rust для работы с внутренним API Max.
Она разработана на основе референса от
[PyMax](https://github.com/MaxApiTeam/PyMax) и наследует те же риски:

* Использование данного инструмента может нарушать User Agreement сервиса Max.
* Автор не несёт ответственности за возможную блокировку ваших аккаунтов,
утерю данных или иные последствия.
...
Сессия сохраняется в `session.db` внутри `work_dir`
(по умолчанию текущая директория). При наличии токена повторный вход не требуется.
...
`reconnect()` — инициирует переподключение
(требуется запущенный `start()` и `config.reconnect = true`).
...
Спасибо авторам оригинально библиотеки на Python
[PyMax](https://github.com/MaxApiTeam/PyMax) за ее создание,
данный проект по сути является ее копипастом на Rust
